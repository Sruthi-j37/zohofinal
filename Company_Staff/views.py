...
def sort_holder_by_name(request):
    if 'login_id' in request.session:
        log_id = request.session['login_id']
        if 'login_id' not in request.session:
            return redirect('/')
        ...
        data=BankAccount.objects.filter(login_details=log_details).order_by('customer_name')
        context = {
            'details': dash_details,
            'allmodules': allmodules,
            'data': data,
...
def sort_holder_by_bank(request):
    if 'login_id' in request.session:
        log_id = request.session['login_id']
        if 'login_id' not in request.session:
            return redirect('/')
        ...
        data=BankAccount.objects.filter(login_details=log_details).order_by('bankname')
        context = {
            'details': dash_details,
            'allmodules': allmodules,
            'data': data,
...
def active_holders(request):
    if 'login_id' in request.session:
        ...
        data=BankAccount.objects.filter(login_details=log_details,status='Active').order_by('-id')
        context = {
            'details': dash_details,
            'allmodules': allmodules,
            'data': data,
...
def createbank(request):
    if request.method == 'POST':
        ...
        if log_details.user_type == 'Company':
            ...
            b_account = request.POST["accountnumber"]
            b_ifsccode = request.POST["ifsc_code"]
            b_branch = request.POST["branch_name"]
            
            bank_create = Banking(
                bnk_name=bank, bnk_acno=b_account, 
                bnk_ifsc=b_ifsccode,
                bnk_branch=b_branch,
        ...
        if log_details.user_type == 'Staff':
            staff_details = StaffDetails.objects.get(login_details=log_details)
            company_details = staff_details.company
            bank = request.POST["bank_name"]
            b_account = request.POST["accountnumber"]
            b_ifsccode = request.POST["ifsc_code"]
            b_branch = request.POST["branch_name"]
            ...
            bank_create = Banking(
                bnk_name=bank, bnk_acno=b_account, bnk_ifsc=b_ifsccode,
                bnk_branch=b_branch,
                company=company_details,
                login_details=log_details
            )
    ...
    if request.method == 'POST':
        if 'login_id' in request.session:
            ...
            if log_details.user_type == 'Company':
                ...
                staff_details = StaffDetails.objects.get(login_details=log_details)
                company_details = staff_details.company
                print(f"Staff Details: {company_details}")
        ...
        return redirect('create_bankholder')
    
def overview_bankholder(request):
    return render(request,'zohomodules/bankholder/overview_bankholder.html')
